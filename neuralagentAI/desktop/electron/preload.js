...
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  // Flowith IPC
  flowithSeekOnce: (payload) => ipcRenderer.invoke('flowith-seek-once', payload),
  flowithSeekStream: (payload) => ipcRenderer.send('flowith-seek-stream', payload),
  onFlowithStreamStart: (cb) => ipcRenderer.on('flowith-stream-start', (_, requestId) => cb(requestId)),
  onFlowithStreamEvent: (cb) => ipcRenderer.on('flowith-stream-event', (_, data) => cb(data)),
  onFlowithStreamEnd: (cb) => ipcRenderer.on('flowith-stream-end', (_, requestId) => cb(requestId)),
  onFlowithStreamError: (cb) => ipcRenderer.on('flowith-stream-error', (_, data) => cb(data)),
});
