...
import { flowithSeekOnce, flowithSeekStream } from './electron/utils/flowith.js';
...
// Flowith bridge (secure, runs in main process)
ipcMain.handle('flowith-seek-once', async (_, payload) => {
  try {
    const { message, kbList, model } = payload || {};
    const result = await flowithSeekOnce({ message, kbList, model });
...
ipcMain.on('flowith-seek-stream', async (event, payload) => {
  const { message, kbList, model } = payload || {};
  const requestId = uuidv4();
  event.sender.send('flowith-stream-start', requestId);
  try {
    await flowithSeekStream({
      message,
      kbList,
      model,
      onEvent: (evt) => {
        event.sender.send('flowith-stream-event', { requestId, ...evt });
      },
    });
    event.sender.send('flowith-stream-end', requestId);
  } catch (err) {
    event.sender.send('flowith-stream-error', { requestId, error: err?.message || String(err) });
  }
});
