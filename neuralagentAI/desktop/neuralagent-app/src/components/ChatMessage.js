...
export default function ChatMessage({ message }) {
  ...
  const getContent = () => {
    ...
    if (type === 'normal_message') {
      const hasImages = Array.isArray(message.attachments) && message.attachments.some(a => a.type === 'image' && a.dataUrl);
      return (
        <>
          {raw}
          {hasImages && (
            <AttachmentPreview>
              {message.attachments.filter(a => a.type === 'image' && a.dataUrl).map((a, idx) => (
                <img src={a.dataUrl} alt={a.name || `image-${idx}`} key={`attimg-${idx}`} />
              ))}
            </AttachmentPreview>
          )}
        </>
