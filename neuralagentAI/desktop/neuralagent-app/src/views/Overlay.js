...
import arkaios_logo_ic_only_white from '../assets/arkaios_logo_ic_only_white.png'
...
export default function Overlay() {
  ...
  const toggleOverlay = async () => {
    if (!expanded) {
      if (runningThreadId === null) {
        if (window.electronAPI?.expandOverlay) {
  ...
  const getSuggestions = async () => {
    try {
      if (window.electronAPI?.getSuggestions) {
        ...
        // Fallback básico para web preview
        setSuggestions([
          { title: 'Open a local project', ai_prompt: 'Open my project folder and list TODOs' },
          { title: 'System status', ai_prompt: 'Check local services status' },
          { title: 'Create task draft', ai_prompt: 'Draft a task to automate a daily routine' },
        ]);
      }
    } catch {
      setSuggestions([]);
  ...
  const createThread = async (prompt = null) => {
    ...
    // En modo BYPASS_LOGIN evitamos llamadas al gateway desde el overlay
  ...
  const onBGModeToggleChange = async (value) => {
    // En web preview, electronAPI no existe: solo actualizamos estado local
    ...
    if (value) {
      const ready = await window.electronAPI.isBackgroundModeReady();
      if (!ready) {
        if (window.electronAPI?.startBackgroundSetup) {
    ...
    <Container>
      <div style={{display: 'flex', alignItems: 'center', width: '100%', height: '60px'}}>
        <AvatarButton onClick={() => toggleOverlay()}>
          <img
            src={arkaios_logo_ic_only_white}
            alt='GOD ARKAIOS AI'
            height={30}
        ...
        {expanded && (
          <>
            ...
            <Input
              ...
              placeholder="Ask GOD ARKAIOS AI..."
